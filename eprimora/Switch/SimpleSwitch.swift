...
public struct SimpleSwitchConfig {
...
@IBDesignable open class SimpleSwitch: BaseSwitch {
    ...
    private var lineWidth : CGFloat { return CGFloat(lineSize) * sizeScale }
    ...
    open func changeValueAnimate(_ turnOn:Bool, duration:Double) {
        ...
        backgroundFillColorAnim.values   = [stateToFillColor(!turnOn), stateToFillColor(!turnOn), stateToFillColor(turnOn), stateToFillColor(turnOn)]
    ...
    private func stateToFillColor(_ isOn:Bool) -> CGColor { return isOn ?  onColor.cgColor : offColor.cgColor }
}

extension SimpleSwitch {
    
    open var config: SimpleSwitchConfig {
        
        set {
            
            if newValue.onColor != onColor { onColor = newValue.onColor }
            if newValue.offColor != offColor { offColor = newValue.offColor }
            if newValue.lineColor != lineColor { lineColor = newValue.lineColor }
            if newValue.circleColor != circleColor { lineColor = newValue.lineColor }
            if newValue.lineSize != lineSize { lineSize = newValue.lineSize }
        }
        
        get {
            
            return SimpleSwitchConfig(onColor: onColor, offColor: offColor, lineColor: lineColor, circleColor: circleColor, lineSize: lineSize)
        }
    }
}
