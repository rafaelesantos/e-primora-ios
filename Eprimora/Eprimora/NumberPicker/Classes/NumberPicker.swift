...
public class NumberPicker: UIViewController {
    ...
    var cellWidthIncludingSpacing: CGFloat { return layout.itemSize.width + layout.minimumLineSpacing }
    ...
    var selectedNumber: Int = 0 {
        didSet { self.numberLbl.text = "\(selectedNumber)" }
    }
    ...
    public func setIndexPath(indexPath: IndexPath) { self.tag = indexPath }
    ...
    func addViews() {
        ...
        if let window = UIApplication.shared.keyWindow { bottomPadding = window.safeAreaInsets.bottom }
    ...
    @objc func btnTapped(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
            ...
            self.dismiss(animated: true, completion: {
                ...
                if sender.tag == 99 { self.delegate.selectedNumber(self.selectedNumber, self.tag) }
            })
        }
    }
    ...
    func getImageFromBundle(_ name: String) -> UIImage {
        ...
        if let image = UIImage(named: name, in: primaryBundle, compatibleWith: nil) { return image }
        else if
...
extension NumberPicker: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! NumberPickerLineCell
